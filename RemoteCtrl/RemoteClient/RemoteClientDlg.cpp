...
{
	return CClientController::getInstance()->SendCommandPacket(GetSafeHwnd(), nCmd, bAutoClose, pData, nLength);
}
...
void CRemoteClientDlg::LoadFileInfo()
{
	...
	HTREEITEM hTreeSelected = m_Tree.HitTest(ptMouse, 0);//获取鼠标点击位置
	...
	CString strPath = GetPath(hTreeSelected);//拿到该文件的完整路径
	...
	{
		TRACE("listPackets.size = %d/r/n", lstPackets.size());
...
void CRemoteClientDlg::InitUIData()
{
	...
	m_nPort = _T("9527");
	CClientController::getInstance()->UpdateAddress(m_server_address, atoi((LPCSTR)m_nPort));
	UpdateData(FALSE);
...
void CRemoteClientDlg::LoadFileCurrent()
{
	...
	while (pInfo->HasNext)
	{
		...
		int cmd = CClientController::getInstance()->DealCommand();//再收一次表示结束
		if (cmd < 0)
		{
			break;
		}
...
void CRemoteClientDlg::OnBnClickedBtnTest()
{
	...
	TRACE(__FUNCTION__, __LINE__, "ret=%d\r\n", ret);
	TRACE("ret=%d\r\n", ret);
...
void CRemoteClientDlg::OnDownloadFile()
{
	...
	strFile = this->GetPath(hSelected) + strFile;
...
void CRemoteClientDlg::OnDeleteFile()
{
	...
	int ret = CClientController::getInstance()->SendCommandPacket(GetSafeHwnd(), 9, true, (BYTE*)(LPCSTR)strFile);
...
LRESULT CRemoteClientDlg::OnSendPackAck(WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		if (wParam != NULL)
		{
			...
			delete (CPacket*)wParam;//避免内存泄漏，因为为了跨线程传递pack是new出来的
